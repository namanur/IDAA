...
- `bun run admin:set <email> <password>`: Promote user to admin, force-verify, and set password.

## 🔐 Authentication & Admin Management

### Admin Setup
To promote a user to admin and bypass email verification locally:
```bash
bun run admin:set namanurwar092@gmail.com your_secure_password
```
This script uses the Supabase Admin API to update the `user_profiles` role, mark the email as confirmed, and set the password.

### Email Verification Issues
If users are receiving `localhost:3000` links in their email:
1. Go to **Supabase Dashboard** > **Authentication** > **URL Configuration**.
2. Update **Site URL** to: `https://idaa-gamma.vercel.app/`
3. Update **Redirect URLs** to include: `https://idaa-gamma.vercel.app/**`

To disable email confirmation for easier local development:
