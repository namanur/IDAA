...
- **Tasks**:
  - [x] **Admin Dashboard**: List topics, trigger Gemini, review markdown.
  - [x] **Generation Engine**: Implement retry logic and version promotion.
  - [x] **Locking Mechanism**: Atomic updates for race condition prevention.
  - [ ] **Scheduler**: Daily Cron for automatic state progression (TODO: Edge Function).
- **Validation**: Admin can trigger generation, see status change to `generating`, then `generated`.
